...
<!-- Weekly Comparison & Top Staff -->
<div class="row mb-4 g-3">
    <div class="col-12 col-lg-6">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-light">
                <h6 class="mb-0">
                    <i class="bi bi-calendar-week text-info me-2"></i>Perbandingan Minggu Ini vs Minggu Lalu
                </h6>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-6">
                        <div class="text-center p-3 bg-success bg-opacity-10 rounded">
                            <i class="bi bi-arrow-up-circle fs-3 text-success mb-2"></i>
                            <h4 class="mb-1">{{ number_format($stats['week_stock_in'] ?? 0) }}</h4>
                            <p class="text-muted small mb-1">Stock In Minggu Ini</p>
                            @php
                                $weekInChange = ($stats['last_week_stock_in'] ?? 0) > 0 
                                    ? (($stats['week_stock_in'] ?? 0) - ($stats['last_week_stock_in'] ?? 0)) / ($stats['last_week_stock_in'] ?? 1) * 100 
                                    : 0;
                            @endphp
                            @if($weekInChange > 0)
                                <small class="badge bg-success">
                                    <i class="bi bi-arrow-up"></i> {{ number_format(abs($weekInChange), 1) }}%
                                </small>
                            @elseif($weekInChange < 0)
                                <small class="badge bg-danger">
                                    <i class="bi bi-arrow-down"></i> {{ number_format(abs($weekInChange), 1) }}%
                                </small>
                            @else
                                <small class="badge bg-secondary">Stabil</small>
                            @endif
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="text-center p-3 bg-danger bg-opacity-10 rounded">
                            <i class="bi bi-arrow-down-circle fs-3 text-danger mb-2"></i>
                            <h4 class="mb-1">{{ number_format($stats['week_stock_out'] ?? 0) }}</h4>
                            <p class="text-muted small mb-1">Stock Out Minggu Ini</p>
                ...
                <div class="mt-3 p-3 bg-light rounded">
                    <div class="d-flex justify-content-between align-items-center">
                        <span class="text-muted small">Net Change:</span>
                        @php
                            $netChange = ($stats['week_stock_in'] ?? 0) - ($stats['week_stock_out'] ?? 0);
                        @endphp
        ...
        <div class="card border-0 shadow-sm h-100 border-start border-primary border-3">
            <div class="card-header bg-primary bg-opacity-10">
                <h6 class="mb-0 text-primary">
                    <i class="bi bi-graph-up-arrow me-2"></i>Item Paling Aktif (Bulan Ini)
                </h6>
            </div>
            <div class="card-body">
                @if(isset($activeItems) && $activeItems->count() > 0)
                    <p class="text-muted small mb-3">Item dengan pergerakan stok tertinggi bulan ini</p>
                    <div class="list-group list-group-flush">
                        @foreach($activeItems as $index => $item)
                            <div class="list-group-item px-0 py-3 border-0">
                                <div class="d-flex align-items-start">
                                    <div class="flex-shrink-0 me-3">
                                        <div class="bg-gradient-primary text-white rounded d-flex align-items-center justify-content-center" style="width: 45px; height: 45px;">
                                    ...
                                    <div class="flex-grow-1">
                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                            <div>
                                                <h6 class="mb-1">{{ Str::limit($item->name, 30) }}</h6>
                                                <span class="badge bg-secondary">{{ $item->code }}</span>
                                            </div>
                                            <span class="badge bg-primary fs-6">
                                                {{ number_format($item->total_movement) }} {{ $item->unit }}
                                            </span>
                                        </div>
                                        <div class="progress" style="height: 8px;">
                                            @php
                                                $maxMovement = $activeItems->first()->total_movement ?? 1;
                                                $progressPercentage = ($item->total_movement / $maxMovement) * 100;
                                            @endphp
                                        ...
                                        <div class="d-flex justify-content-between mt-2">
                                            ...
                                            <small class="fw-bold">{{ number_format($item->quantity) }} {{ $item->unit }}</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="mt-3 text-center">
                        <a href="{{ route('gudang.stock.movement') }}" class="btn btn-primary btn-sm">
                            <i class="bi bi-activity me-2"></i>Lihat Riwayat Pergerakan
                        </a>
                    </div>
                @else
                    <div class="text-center py-4">
